...
public class UserDao {
	...
	private static String CREATE = "insert into User(user_firstname, user_lastname, user_age, user_email) value(?,?,?,?)";
	private static String READ_BY_ID = "select * from User where id = ?";
	private static String UPDATE_BY_ID = "update User set user_firstname =?, user_lastname=?, user_age=?, user_email=? where id= ?";
	...
	public void update(User user) throws SQLException {
		...
		preparedStatemant.setInt(3, user.getUser_age());
		preparedStatemant.setString(4, user.getUser_email());
		preparedStatemant.setInt(5, user.getId());
		preparedStatemant.executeUpdate();
	}
